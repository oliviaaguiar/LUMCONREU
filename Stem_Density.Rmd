...
plants.live %>% ggplot(aes(x = date, y = mean)) + geom_point(aes(c = date, y = mean)) + geom_errorbar(aes(ymin = mean-SE, ymax = mean+SE)) + xlab('Date') + ylab('Stem Density') + ggtitle('Live Stem Density') + theme_bw() + theme(axis.text.x = element_text(size = 7, angle = 45), axis.ticks = element_blank())
```
